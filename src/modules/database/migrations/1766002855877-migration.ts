...
export class Migration1766002855877 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."plan_prices_interval_enum" AS ENUM('month', 'year', 'week', 'day')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "plans" ADD "active" boolean NOT NULL DEFAULT true`,
    );
    await queryRunner.query(
      `ALTER TABLE "plans" ADD "category" character varying`,
    );
    await queryRunner.query(`ALTER TABLE "plans" ADD "features" jsonb`);
    await queryRunner.query(
      `ALTER TABLE "subscriptions" ADD "active" boolean NOT NULL DEFAULT true`,
    );
    await queryRunner.query(
      `ALTER TABLE "subscriptions" ADD "startDate" TIMESTAMP WITH TIME ZONE`,
    );
    await queryRunner.query(
      `ALTER TABLE "subscriptions" ADD "canceledAt" TIMESTAMP WITH TIME ZONE`,
    );
    await queryRunner.query(
      `ALTER TABLE "subscriptions" ADD "expiresAt" TIMESTAMP WITH TIME ZONE`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "plan_prices" ADD CONSTRAINT "FK_eb2f222f91a8e78e9e1d591b0de" FOREIGN KEY ("planId") REFERENCES "plans"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "subscriptions" ADD CONSTRAINT "FK_adc20a425b6268a729c11f87a18" FOREIGN KEY ("priceId") REFERENCES "plan_prices"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "subscriptions" DROP CONSTRAINT "FK_adc20a425b6268a729c11f87a18"`,
    );
    await queryRunner.query(
